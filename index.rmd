...
author: 'Jun-Sok Huhh [`r fontawesome::fa("envelope", fill="white", height=12)`](mailto:anarinsk@gmail.com) [`r fontawesome::fa("home", fill="white", height=12)`](http://lostineconomics.com) [`r fontawesome::fa("github", fill="white", height=12)`](http://github.com/anarinsk)' 
output:
...
Adoption behavior of individual users is also skewed to 0.[^adoption] The distribution of play time is somewhat uniformly distributed in log sense. That is, a considerable proportion of players, 29.3%, consumes time spent on games more than 100 hours. The above table shows that distribution of sales and playing time by title is very skewed to 0. That is, a small number of titles occupy most of the adoption and play time of users.

[^adoption]: Instead of purchasing, we use adoption as users' behavior to choose games with or without paying. As there are many free-to-play games In our data sample, adoption is more proper in our discussion. 
...
p1 <- df_id %>% 
  mutate(
    purchased_rescaled1 = (n_purchased - min(n_purchased)) / min(n_purchased), 
    purchased_rescaled2 = log10(n_purchased)
  ) %>% 
  filter(
    between(purchased_rescaled2, 0, 1.5)
  ) %>% 
  ggplot() + 
  aes(x=n_purchased, y=sh_play) +
...
p2 <- df_id %>% 
  ...
  filter(
    between(purchased_rescaled2, 1.500001, 3)
  ) %>% 
  ggplot() + 
  aes(x=log10(n_purchased), y=sh_play) +
  geom_point() + 
