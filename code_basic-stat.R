...
librarian::shelf(rlang, qwraps2, janitor, stringr, knitr, kableExtra, scales, thomasp85/patchwork)
...
c("n_purchased", "time") %>% map_df(function(x){get_unitsummary(df_title, x)}) %>% kable() %>% kable_styling()
...
df_id %>% arrange(desc(n_purchased)) %>% top_n(20) %>% 
  kable() %>% kable_styling()
